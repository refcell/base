...
| --- | --- | --- | --- | --- |
| M0 | Unchanged baseline build, tests, full devnet and verify-base | — | Supervisor: sole remote executor | Build/images, inclusion, validator agreement, graceful restart and bounded finality observations pass. Full baseline suite remains 8620pass/23fail/83skip; corrected failure classification in evt-0007. |
...
| M2 | Bootstrap charter, roadmap, ownership, ADRs, behavior inventory and evidence validator | — | Supervisor / file-only authors | Integrated; validator17tests and independent review pass. Draft fork PR #1 is open. |
...
| M4 | Unified real load/drain/rescue/config/output/shutdown; retire standalone product after parity | M0 exercised/failures dispositioned; M3 | Supervisor + file-only authors/reviewers | M4-A integrated at450b4d7b6: source-built full candidate devnet/restart, load/SIGINT/rescue and17focused tests pass; full suite8635pass/25fail/83skip remains failed. M4-B pending: real-token/mnemonic/TTY checks and wrapper removal. |
