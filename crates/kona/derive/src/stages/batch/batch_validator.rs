...
mod test {
    ...
    async fn test_batch_validator_l1_origin_mismatch() {
        ...
        bv.l1_blocks.push(BlockInfo { number: 5, ..Default::default() });
        bv.l1_blocks.push(BlockInfo { number: 6, ..Default::default() });

        let mock_parent = L2BlockInfo {
            l1_origin: BlockNumHash { number: 8, hash: [0xFF; 32].into() },
            ..Default::default()
        };

        let err = bv.next_batch(mock_parent).await.unwrap_err();
        assert!(
            matches!(err, PipelineErrorKind::Reset(ResetError::L1OriginMismatch(_, _))),
            "Expected L1OriginMismatch, got {err:?}"
        );
    }
