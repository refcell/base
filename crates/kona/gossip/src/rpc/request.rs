...
pub enum P2pRpcRequest {
    ...
    /// Dumps the node's discovery table from the `Discv5Driver`.
    DiscoveryTable(Sender<Vec<String>>),
    /// Returns the current peer count for both discovery and gossip services.
    ...
    /// Returns the current peer stats for both discovery and gossip services.
            ...
            let node_to_peer_id: HashMap<NodeId, PeerId> = peer_ids.into_iter().filter_map(|id|
            {
                ...
                match pubkey.try_into_secp256k1().map_err(|err| err.to_string()).and_then(
                    |key| ecdsa::VerifyingKey::from_sec1_bytes(key.to_bytes().as_slice()).map_err(|err| err.to_string())
                        ...
                        Err(err) => {
                            error!(target: "p2p::rpc", peer_id = ?id, err = ?err, "Failed to convert public key to secp256k1 public key.");
                            return None;
                        }};
    ...
    fn handle_peer_stats<G: ConnectionGate>(
        ...
        tokio::spawn(async move {
            ...
            let Ok(table) = table.try_into() else {
                error!(target: "p2p::rpc", "failed to get discovery table size. Integer overflow.");
                return;
            };

            let Ok(connected) = gossip_network_info.num_peers().try_into() else {
                error!(target: "p2p::rpc", "failed to get number of connected peers. Integer overflow.");
                return;
            };

            let Ok(known) = peers_known.try_into() else {
                error!(target: "p2p::rpc", "failed to get number of known peers. Integer overflow.");
                return;
            };
            ...
            .collect::<Result<Vec<_>, _>>() else {
                error!(target: "p2p::rpc", "failed to get blocks topic. Some topic count overflowed.");
                return;
            };
