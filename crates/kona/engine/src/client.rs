    ...
    L2Provider: Provider<Optimism>,
{
    ...
    async fn fork_choice_updated_v3(
        ...
        let op_attrs = rollup_boost_compat::to_op_payload_attrs(payload_attributes);
        let call = self.rollup_boost.fork_choice_updated_v3(fork_choice_state, op_attrs);
    ...
    async fn get_payload_v3(
        ...
        let result = record_call_time(call, Metrics::GET_PAYLOAD_METHOD)
            .await
            .map_err(|err| RollupBoostServerError::from(err))?;

        Ok(rollup_boost_compat::from_op_envelope_v3(result))
    }
    ...
    async fn get_payload_v4(
        ...
        Ok(rollup_boost_compat::from_op_envelope_v4(result))
    }
...
mod rollup_boost_compat {
    ...
    pub(super) fn to_op_payload_v4(
        payload: &OpExecutionPayloadV4,
    ) -> op_alloy_rpc_types_engine::OpExecutionPayloadV4 {
        convert(payload)
    }

    pub(super) fn to_op_payload_attrs(
        attrs: Option<OpPayloadAttributes>,
    ) -> Option<op_alloy_rpc_types_engine::OpPayloadAttributes> {
        attrs.map(|a| convert(&a))
    }

    pub(super) fn from_op_envelope_v3(
        envelope: op_alloy_rpc_types_engine::OpExecutionPayloadEnvelopeV3,
    ) -> OpExecutionPayloadEnvelopeV3 {
        convert(&envelope)
    }

    pub(super) fn from_op_envelope_v4(
        envelope: op_alloy_rpc_types_engine::OpExecutionPayloadEnvelopeV4,
