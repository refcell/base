    ...
    pub(super) async fn handle_peer_monitoring(&mut self) {
        ...
        let peers_to_remove = self
            ...
            .filter_map(|peer_id| {
                ...
                base_macros::record!(
                    histogram,
                    base_gossip::Metrics::PEER_SCORES,
                    "peer",
                    peer_id.to_string(),
                    score
                );
        ...
        let addrs_to_ban = peers_to_remove
            ...
            .filter_map(|peer_to_remove| {
                ...
                if let Some(_start_time) = self.gossip.peer_connection_start.remove(&peer_to_remove) {
                    base_macros::record!(
                        histogram,
                        base_gossip::Metrics::GOSSIP_PEER_CONNECTION_DURATION_SECONDS,
                        _start_time.elapsed().as_secs_f64()
                    );
                }

                if let Some(info) = self.gossip.peerstore.remove(&peer_to_remove) {
                    use base_gossip::ConnectionGate;
                    self.gossip.connection_gate.remove_dial(&peer_to_remove);
                    let _peer_score = self.gossip.swarm.behaviour().gossipsub.peer_score(&peer_to_remove).unwrap_or_default();
                    base_macros::inc!(gauge, base_gossip::Metrics::BANNED_PEERS, "peer_id" => peer_to_remove.to_string(), "score" => _peer_score.to_string());
